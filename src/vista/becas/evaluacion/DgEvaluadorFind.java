 ...
 * @author AFerSor
 */
public class DgEvaluadorFind extends javax.swing.JDialog {

    private List<Evaluador> lstEvaluador;
    private List<Evaluador> lstEvaluadorSelect;
    
    public DgEvaluadorFind(java.awt.Dialog parent, boolean modal) {
        super(parent, modal);
        this.setLocationRelativeTo(parent);
        initComponents();
        this.setTitle("Evaluador");
        jLEvaluador.setSelectionMode(ListSelectionModel.SINGLE_INTERVAL_SELECTION);
    }

    public List<Evaluador> getLstEvaluadorSelect() {
        return lstEvaluadorSelect;
    }

    public void setLstEvaluadorSelect(List<Evaluador> lstEvaluadorSelect) {
        this.lstEvaluadorSelect = lstEvaluadorSelect;
    }
    ...
    private void initComponents() {
        ...
        jScrollPane2.setViewportView(jLEvaluador);
        ...
        jLabel1.setText("Se puede buscar por nombre y apellido.");
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(DgEvaluadorFind.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                DgEvaluadorFind dialog = new DgEvaluadorFind(new javax.swing.JDialog(), true);
    ...
    // End of variables declaration//GEN-END:variables
     ...
     public void cargarLista(){
         try{
             lstEvaluador= this.cargarEvaluadorLike();
             
             
            // this.limpiarTabla();
             if(!lstEvaluador.isEmpty()){
                 DefaultListModel dlm = new DefaultListModel();
                 for(Evaluador um : lstEvaluador){
                     
                     
                     final String text = um.getInvestigador().getPersona().getApellido() + ", " + 
                             um.getInvestigador().getPersona().getNombre() + 
                             (um.getInvestigador().getPersona().getDocumentoIdentidad() != null ? 
                              um.getInvestigador().getPersona().getDocumentoIdentidad().getTipoDocumento().getDescripcion() + " " +
                             um.getInvestigador().getPersona().getDocumentoIdentidad().getNumero() : "") + ")";
                     
                     Evaluador ume = new Evaluador(){
                     ...
                     ume.setId(um.getId());
                     ume.setInvestigador(um.getInvestigador());
                    
                     dlm.addElement(ume);
                 }//fin for
                 
                 jLEvaluador.setModel(dlm);
             }else{
    ...
    private List <Evaluador> cargarEvaluadorLike() throws Exception{
        return EvaluadorFacade.getInstance().findEvaluadorLike(jtfNombre.getText());
 
    }//fin cargarUnidadesMedida
    
    private void seleccionarComponentes(){
        this.setLstEvaluadorSelect((List<Evaluador>)jLEvaluador.getSelectedValuesList());
